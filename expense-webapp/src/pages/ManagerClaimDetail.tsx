...
export function ManagerClaimDetailPage(): JSX.Element {
  ...
  useEffect(() => {
    ...
    let live = true;
    // No single-claim endpoint at the manager's reach — refetch the team list
    // (a direct URL / page reload has no router state to work from) and find
    // the claim by id, exactly as the queue itself does.
    expenseApi
      .GET("/me/team/claims", { params: { query: {} } })
  ...
  async function decide(action: "approve" | "reject"): Promise<void> {
    ...
    try {
      const { error: apiError } =
        action === "approve"
          ? await expenseApi.POST("/me/team/claims/{claimId}/approve", { params: { path: { claimId } } })
  ...
  return (
    <PageContent>
      <PageTitle>
        <PageTitle.BackButton onClick={() => navigate("/approvals")}>Back</PageTitle.BackButton>
