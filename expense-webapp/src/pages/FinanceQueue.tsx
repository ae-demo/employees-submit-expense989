...
export function FinanceQueuePage(): JSX.Element {
  ...
  useEffect(() => {
    let live = true;
    expenseApi
      .GET("/claims", { params: { query: { status: "manager-approved" } } })
      ...
      .catch((err: unknown) => {
        if (live) setError(err instanceof Error ? err.message : "Failed to load the finance queue");
      });
    return () => {
      live = false;
    };
  ...
  return (
    <PageContent>
      <PageTitle>
        <PageTitle.Header>Finance Review Queue</PageTitle.Header>
      </PageTitle>
          ...
          <ListingTable.Body>
              ...
              <ListingTable.Row>
                <ListingTable.Cell colSpan={5}>
                  <ListingTable.EmptyState
                    title="Nothing awaiting finance review"
                    description="No manager-approved claims are waiting right now."
                  />
                </ListingTable.Cell>
              </ListingTable.Row>
            ) : null}
            {claims?.map((claim) => (
              <ListingTable.Row
                key={claim.id}
                clickable
                onClick={() => navigate(`/finance/${claim.id}`, { state: { claim } })}
              >
